...
# Load and scale desert background
menu_image_file = 'menu.jpg'  # Ensure this file exists in the same directory

if not os.path.exists(menu_image_file):
    print("Error: 'menu.jpg' not found. Please place it in the game directory.")
else:
    menu_background = pygame.image.load(menu_image_file).convert()
    menu_background = pygame.transform.scale(menu_background, (display_width, display_height))

# Load and scale desert background
desert_image_file = 'desert.png'  # Ensure this file exists in the same directory

if not os.path.exists(desert_image_file):
    print("Error: 'desert.png' not found. Please place it in the game directory.")
else:
    desert_background = pygame.image.load(desert_image_file).convert()
    desert_background = pygame.transform.scale(desert_background, (display_width, display_height))
...
vsmallfont = pygame.font.SysFont("comicsansms", 25)
...
def game_controls():
    ...
    while gcont:
        ...
        message_to_screen("Press D to raise Power % AND Press A to lower Power % ", black, 140)
        message_to_screen("Pause: P", black, 90)

        button("Play", 150, 500, 100, 50, black, light_green, action="play")
        button("Main", 350, 500, 100, 50, black, light_yellow, action="main")
        button("Quit", 550, 500, 100, 50, black, light_red, action="quit")
...
def button(text, x, y, width, height, inactive_color, active_color, action=None, size="small"):
    ...
    color = active_color if (x < cur[0] < x + width and y < cur[1] < y + height) else inactive_color
    pygame.draw.rect(gameDisplay, color, (x, y, width, height))

    # Check for click and trigger the action
...
def fireShell(xy, tankx, tanky, turPos, gun_power, xlocation, barrier_width, randomHeight, enemyTankX, enemyTankY):
    ...
    while fire:
        ...
        pygame.display.update()  # Update the screen to show the trajectory
    ...
    # Check for hit on the enemy tank
            if enemyTankX - (tankWidth // 2) <= hit_x <= enemyTankX + (tankWidth // 2):
                ...
                print("Hard Hit!")
                damage = 18
            elif enemyTankX - (tankWidth // 2 + 20) <= hit_x <= enemyTankX + (tankWidth // 2 + 20):
                print("Medium Hit")
                damage = 10
            elif enemyTankX - (tankWidth // 2 + 30) <= hit_x <= enemyTankX + (tankWidth // 2 + 30):
                print("Light Hit")
                damage = 5

    # Trigger explosion at the impact point
            explosion(hit_x, hit_y)
            fire = False


        # Add a small delay to slow down the projectile
        clock.tick(60)
...
def gameLoop():
    ...
    xlocation = (display_width // 2) + random.randint(int(-0.1 * display_width), int(0.1 * display_width))
...
# Download and initialize elevator music
# Replace the elevator music URL
# Replace dynamic download with a local file reference
music_file = 'elevator_music.mp3'
# Ensure this file exists in the same directory

if not os.path.exists(music_file):
    print("Error: 'elevator_music.mp3' not found. Please place it in the game directory.")
else:
